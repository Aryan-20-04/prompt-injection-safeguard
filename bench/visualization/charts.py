...
class ChartGenerator:
    ...
    def generate_all(self, result, output_dir: str):
        ...
        self.confusion_matrix(result, result.label_schema, output_dir)
    ...
    def per_attack_radar(self, result, output_dir: str):
        ...
        ax.set_xticklabels(attacks, size=9)
        ax.set_ylim(0, 1)
        ax.set_title(f"Per-Attack F1 — {result.model_name}", pad=20)
        fname = result.model_name.replace("/", "_")
        fig.savefig(f"{output_dir}/{fname}_radar.png", dpi=150, bbox_inches="tight")
        plt.close(fig)
