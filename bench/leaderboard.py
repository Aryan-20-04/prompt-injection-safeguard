...
class LeaderboardGenerator:
    ...
    def _collect_rows(self, results_dir: str) -> List[LeaderboardRow]:
        rows = []
        for p in Path(results_dir).rglob("metrics/*.json"):
            with open(p) as f:
                data = json.load(f)
            meta_path = p.parent.parent / "run_metadata.json"
            meta = json.loads(meta_path.read_text()) if meta_path.exists() else {}
            gm = data["global_metrics"]
            rows.append(LeaderboardRow(
                model=data["model_name"],
                dataset=data["dataset_name"],
                micro_f1=gm["micro_f1"],
    ...
    def _write_csv(self, rows, output_dir):
        p = Path(output_dir) / "leaderboard.csv"
        with open(p, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow([
                "rank", "model", "dataset", "micro_f1", "macro_f1",
                "precision", "recall", "latency_p50", "latency_p95", "run_date",
            ])
            for i, r in enumerate(rows, 1):
                w.writerow([
                    i, r.model, r.dataset,
                    f"{r.micro_f1:.6f}", f"{r.macro_f1:.6f}",
                    f"{r.precision:.6f}", f"{r.recall:.6f}",
                    f"{r.latency_p50:.2f}", f"{r.latency_p95:.2f}",
                    r.run_date,
                ])
